...
/// 时间记录存储服务 V2 - 使用 SQLite
class TimeLoggerStorageV2 {
  ...
  static Future<void> migrateFromOldStorage() async {
    ...
    try {
      // 1. 迁移活动记录
      final oldRecords = await TimeLoggerStorage.getAllRecords();
      ...
      // 2. 迁移活动历史
      final oldHistory = await TimeLoggerStorage.getActivityHistory();
